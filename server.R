...
server <- function(input, output) {
  ...
  output$sidebar_etudes <- renderUI({
    tagList(
      conditionalPanel(
        condition = 'input.tabs=="Résumé des études"',
  ...
  output$sidebar_swimmer <- renderUI({
    tagList(
      conditionalPanel(
        condition = 'input.tabs=="Gestion des projets"',
        sliderInput(
          "swim_year",
          "Période :",
          min = min_year_swim,
          max = max_year_swim,
          value = c(min_year_swim, max_year_swim)
        ),
        selectInput(
          "swim_type_projet",
          "type_projet",
  ...
  output$sidebar_calendar <- renderUI({
    tagList(
      conditionalPanel(
        condition = 'input.tabs=="Calendrier"',
        sliderInput(
          "calendar_year",
          "Année :",
          min = min_year_calendar,
          max = max_year_calendar,
          value = current_year
        ),
        selectInput(
          "calendar_type_projet",
          "type_projet",
          liste_choix_projets,
          multiple = TRUE,
  ...
  filtre_choix_etat <- reactive({
    if (input$choix_etat == "Tout") {
      df$etat_prise_en_charge %>% as.character() %>% unique() %>% sort()
    } else {
      input$choix_etat
    }
  })
  ...
  tib_filtered <- reactive({
    df %>%
      filter(type_projet %in% filtre_choix_projet()) %>%
      filter(annee_contrat %in% input$choix_année[1]:input$choix_année[2]) %>%
      filter(origine %in% filtre_choix_intex()) %>%
      filter(etat_prise_en_charge %in% filtre_choix_etat()) %>%
      select(
        -c(
          annee_contrat,
          organe,
          ...
          if (input$choix_intex == "Interne") {
            c("delai_signature", "date_signature_contrat_convention")
          }
        )
      ) %>%
      mutate_if(
      ...
      mutate_at(
        c("avis_comite_date", "date_debut_saisie", "date_fin_saisie"),
        ~ ymd(.)
      ) %>%
  ...
  my_gt_table <- reactive({
    tbl_summary(
      ...
      tab_header(md(
        str_glue(
          "Données de {input$choix_année[1]} à {input$choix_année[2]} \n
        {input$choix_intex} \n
  ...
  df_swim_filtered_reactive <- reactive({
    df_swim %>%
      ## Filtres dynamiques
      filter(
        year(avis_comite_date) >= filtre_swim_year()[1] &
          year(date_fin_contrat) <= filtre_swim_year()[2]
      ) %>%
      filter(type_projet %in% filtre_swim_projet()) %>%
      filter(etat_prise_en_charge %in% filtre_swim_etat()) %>%
  ...
  df_swim_lanes_reactive <- reactive({
    bind_rows(
      df_swim_filtered_reactive() %>%
        select(
          nom_projet,
          etat_prise_en_charge,
          deb_reglementaire,
          finreg_contrat
        ) %>%
        rename(Debut = deb_reglementaire, Fin = finreg_contrat) %>%
      ...
      df_swim_filtered_reactive() %>%
        ...
        rename(Debut = deb_ges_contrat, Fin = fin_ges_saisie) %>%
      ...
      df_swim_filtered_reactive() %>%
        ...
        rename(Debut = deb_ana_saisie, Fin = fin_ana_proj) %>%
  ...
  df_swim_events_reactive <- reactive({
    df_swim_filtered_reactive() %>%
      ...
      pivot_longer(
        cols = str_subset(colnames(df_swim), "[Dd]ate"),
        names_to = "event",
        values_to = "date"
      ) %>%
      na.omit()
  ...
  df_plot <- reactive({
    df %>%
      filter(type_projet %in% filter_projet_plot()) %>%
  ...
  output$plot_titre <- renderText({
    if (input$choix_année_plot[1] == input$choix_année_plot[2]) {
      ...
      annees <- str_glue(
        "Données de {input$choix_année_plot[1]} à {input$choix_année_plot[2]}"
      )
    }
  ...
  reactive_df_calendar <- reactive({
    ...
    df_calendar_filtered <- df_calendar %>%
      ...
      filter(type_projet %in% filter_projet_calendar())
    ...
    selected_year_calendar <- tibble(
      ...
      group_by(days) %>%
      mutate(
        n_overlaps = sum(days %within% df_calendar_filtered$interval_projet)
      )
  ...
  output$calendar_plot <- renderPlot({
    calendR(
      year = year_calendar(), # Affichage de l'année en haut
